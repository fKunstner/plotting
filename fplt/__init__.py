...
def make_slider(fig, coordinates, label, start, end, init, step):
    """Makes a new axis and puts a slider in coordinates [left,bottom,width,height]
    label, start, end, init and step are passed to the slider construction.
...
def nice_logspace(start, stop, base, density):
    """Returns a log-spaced grid between base**start and base**end.

    Plays nicely with merge_grids.
    ...
    Density = 2 will return
    """
    if density < 1 or not np.allclose(int(density), density):
        raise ValueError(
            f"Density needs to be an integer > 1, got {density}."
        )
    if not np.allclose(int(start), start) or not np.allclose(int(stop), stop):
        raise ValueError(
            f"Start and end need to be integers, got {start, stop}."
        )
    if not (stop > start):
        raise ValueError(
            f"Start needs to be smaller than stop, got {start, stop}."
        )
    assert stop > start
    return np.logspace(start, stop, base=base, num=(stop - start) * (2 ** (density-1)) + 1)
